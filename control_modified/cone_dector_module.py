...
# 좌측에서만 라바콘 탐지 시 오른쪽으로 핸들 조향각 변경
# 우측에서만 라바콘 탐지 시 왼쪽으로 핸들 조향각 변경
# 좌우측 모두 라바콘 탐지 시 탐지 거리 비교 후 좌측 라바콘이 더 가까우면 오른쪽으로, 우측 라바콘이 더 가까우면 왼쪽으로 핸들 조향각 변경
#==================================================

cone_start = False   # cone_start -> 라바콘 회피 모드 Flag
cone_done = False   # cone_done -> 라바콘 회피를 끝냈으면 True
cone_cnt = 0    # cone_cnt -> 라바콘 회피 ON/OFF 전환 카운터
min_left, max_left = 10, 55     # min_left, max_left -> 좌측 장애물 탐지각(최소, 최대)
min_right, max_right = 305, 350     # min_right, max_right -> 우측 장애물 탐지각(최소, 최대)
min_lr_dist = 4    # min_lr_dist -> 좌, 우측 장애물 탐지 최소 거리
min_front_dist = 8     # min_front_dist -> 전방 장애물 탐지 최소 거리
basic_spd = 25    # 기본 주행 속도
angle_increment = 12   # angle_increment -> 조향각 조정 시, 각 if 블록마다 증가하는 각도
